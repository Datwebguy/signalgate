import { fetchLiveMiners } from '../lib/telegraph/catalog.js';
import { askEngine } from '../lib/telegraph/ask.js';
import { evaluateGatePolicy } from '../lib/gate/policy.js';
import { getTelegraphConfig } from '../lib/telegraph/config.js';
...
async function main() {
  ...
  if (receipt.status === 'PAYMENT_REQUIRED' && gateResult.verdict === 'WAIT' && gateResult.paymentSettled === false) {
    console.log('\n>>> STEP 2 PASSED: 402 cleanly captured and rendered as WAIT without mocking! <<<');
  } else {
    console.error('\n>>> STEP 2 FAILED <<<');
    process.exit(1);
  }
